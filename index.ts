...
(async () => {
  ...
  if (process.env.NODE_ENV === "development" && process.env.ENABLE_METRO_PROXY === "true") {
    const metroProxy = createProxyMiddleware({
      target: process.env.METRO_URL || "http://localhost:8081",
      changeOrigin: true,
      ws: true,
      logger: undefined,
    });
    ...
    log(`Dev proxy: non-API requests forwarded to Metro on ${process.env.METRO_URL || "port 8081"}`);
  }
